 ...
 * Domain3Graph.tsx
 * ─────────────────────────────────────────────────────────────────────────────
 * ROBINS-I V2  Domain 3 – Bias due to deviations from intended interventions
 * (Selection of the reported result — some tools call this "selection bias")
 *
 * SOURCE IMAGE: attached_assets/image_1777800338891.png  ← SOURCE OF TRUTH
 *
 * ALGORITHM STRUCTURE:
 *   Two independent sub-graphs (A and B) are assessed separately.
 *   Their results are then combined to produce a domain-level outcome.
 *
 *   ── SUB-GRAPH A (follow-up timing) ──────────────────────────────────────
 *   3.1 = Y/PY  →  Q3.2  →  N/PN/NI: A=LOW  |  Y/PY: A=MODERATE
 *   3.1 = WN/NI  →  A = MODERATE
 *   3.1 = SY     →  A = SERIOUS
 *
 *   ── SUB-GRAPH B (selection on post-intervention characteristics) ─────────
 *   3.3 = N/PN  →  B = LOW
 *   3.3 = NI    →  B = MODERATE
 *   3.3 = Y/PY  →  Q3.4
 *     3.4 = N/PN  →  B = LOW
 ...
 *       3.5 = Y/PY  →  B = SERIOUS
 *
 *   ── COMBINED (worst of A and B) ─────────────────────────────────────────
 *   Both LOW           →  FINAL: LOW
 *   Worst = MODERATE   →  FINAL: MODERATE
 *   At least one SERIOUS  →  ask Q3.6
 *     3.6 = Y/PY     →  MODERATE
 *     3.6 = N/PN/NI  →  Q3.7
 *       3.7 = Y/PY     →  MODERATE
 *       3.7 = N/PN/NI  →  Q3.8
...
// Domain 3 accent colour: orange
const ACCENT = '#ea580c';
const nodeTypes = { q: QuestionNode, o: OutcomeNode };

// ─── Sub-graph intermediate rating helper ─────────────────────────────────────
// Maps a label to a numeric severity for comparison (higher = worse).
...
type Answers = {
  // Sub-graph A
  q31: string | null; // Q3.1 — participants followed from start of intervention?
  q32: string | null; // Q3.2 — early outcome events excluded? (if 3.1=Y/PY)
  // Sub-graph B
  q33: string | null; // Q3.3 — selection based on characteristics after start?
  q34: string | null; // Q3.4 — selection variables associated with intervention? (if 3.3=Y/PY)
  q35: string | null; // Q3.5 — selection variables influenced by outcome? (if 3.4=Y/PY)
  // Combined
  q36: string | null; // Q3.6 — analysis corrected for selection biases? (if serious)
  q37: string | null; // Q3.7 — sensitivity analyses show minimal impact? (if 3.6=N/PN/NI)
  q38: string | null; // Q3.8 — selection biases severe? (if 3.7=N/PN/NI)
};

// ─── Node layout ──────────────────────────────────────────────────────────────
// Left section: Sub-graph A; Middle section: Sub-graph B; Right section: combined
const BASE_NODES: Node[] = [
  // ── Sub-graph A label ────────────────────────────────────────────────────
  // (Shown as a special node — using 'q' type with a visual distinction)
  { id: 'n31', type: 'q', position: { x: 0, y: 70 },
    data: { qid: '3.1', shortLabel: 'Participants followed from start of intervention?', active: false } as QData },
  { id: 'n32', type: 'q', position: { x: 250, y: 10 },
    data: { qid: '3.2', shortLabel: 'Early outcome events excluded?', active: false } as QData },

  // Intermediate A outcomes (shown as outcome nodes to visualise sub-results)
  { id: 'a_low',  type: 'o', position: { x: 530, y: 0  },
    data: { level: 'low',      label: 'A: LOW',      active: false } as OData },
  { id: 'a_mod',  type: 'o', position: { x: 530, y: 100 },
    data: { level: 'moderate', label: 'A: MOD',      active: false } as OData },
  { id: 'a_ser',  type: 'o', position: { x: 530, y: 200 },
    data: { level: 'serious',  label: 'A: SERIOUS',  active: false } as OData },

  // ── Sub-graph B ───────────────────────────────────────────────────────────
  { id: 'n33', type: 'q', position: { x: 0, y: 430 },
    data: { qid: '3.3', shortLabel: 'Selection based on characteristics after start of intervention?', active: false } as QData },
  { id: 'n34', type: 'q', position: { x: 250, y: 500 },
    data: { qid: '3.4', shortLabel: 'Selection variables associated with intervention?', active: false } as QData },
  { id: 'n35', type: 'q', position: { x: 500, y: 500 },
    data: { qid: '3.5', shortLabel: 'Selection variables influenced by outcome?', active: false } as QData },

  // Intermediate B outcomes
  { id: 'b_low',  type: 'o', position: { x: 530, y: 380 },
    data: { level: 'low',      label: 'B: LOW',     active: false } as OData },
  { id: 'b_mod',  type: 'o', position: { x: 530, y: 480 },
    data: { level: 'moderate', label: 'B: MOD',     active: false } as OData },
  { id: 'b_ser',  type: 'o', position: { x: 530, y: 580 },
    data: { level: 'serious',  label: 'B: SERIOUS', active: false } as OData },

  // ── Combined questions ────────────────────────────────────────────────────
  { id: 'n36', type: 'q', position: { x: 780, y: 290 },
    data: { qid: '3.6', shortLabel: 'Analysis corrected for selection biases?', active: false } as QData },
  { id: 'n37', type: 'q', position: { x: 980, y: 290 },
    data: { qid: '3.7', shortLabel: 'Sensitivity analyses demonstrate minimal impact?', active: false } as QData },
  { id: 'n38', type: 'q', position: { x: 1180, y: 290 },
    data: { qid: '3.8', shortLabel: 'Selection biases severe?', active: false } as QData },

  // ── Final outcomes ────────────────────────────────────────────────────────
  { id: 'o_low',  type: 'o', position: { x: 1420, y: 0   },
    data: { level: 'low',      label: 'LOW',      active: false } as OData },
  { id: 'o_mod',  type: 'o', position: { x: 1420, y: 150 },
    data: { level: 'moderate', label: 'MODERATE', active: false } as OData },
  { id: 'o_ser',  type: 'o', position: { x: 1420, y: 330 },
    data: { level: 'serious',  label: 'SERIOUS',  active: false } as OData },
  { id: 'o_crit', type: 'o', position: { x: 1420, y: 480 },
...
const BASE_EDGES: Edge[] = [
  // ── Sub-graph A ──────────────────────────────────────────────────────────
  mkEdge('e31_32',    'n31', 'n32',  'Y/PY'),  // followed from start → ask about early exclusions
  mkEdge('e31_amod',  'n31', 'a_mod','WN/NI'), // weakly/not followed → MODERATE
  mkEdge('e31_aser',  'n31', 'a_ser','SY'),    // strongly not → SERIOUS

  mkEdge('e32_alow',  'n32', 'a_low', 'N/PN/NI'), // no early exclusions → LOW
  mkEdge('e32_amod',  'n32', 'a_mod', 'Y/PY'),    // early exclusions present → MODERATE

  // ── Sub-graph B ──────────────────────────────────────────────────────────
  mkEdge('e33_blow',  'n33', 'b_low', 'N/PN'), // no post-start selection → LOW
  mkEdge('e33_bmod',  'n33', 'b_mod', 'NI'),   // uncertain → MODERATE
  mkEdge('e33_34',    'n33', 'n34',   'Y/PY'), // post-start selection → check associations

  mkEdge('e34_blow',  'n34', 'b_low', 'N/PN'), // not associated with intervention → LOW
  mkEdge('e34_bmod',  'n34', 'b_mod', 'NI'),   // uncertain → MODERATE
  mkEdge('e34_35',    'n34', 'n35',   'Y/PY'), // associated → check outcome influence

  mkEdge('e35_bmod',  'n35', 'b_mod', 'N/PN'), // not influenced by outcome → MODERATE
  mkEdge('e35_bmod2', 'n35', 'b_mod', 'NI'),   // uncertain → MODERATE
  mkEdge('e35_bser',  'n35', 'b_ser', 'Y/PY'), // influenced by outcome → SERIOUS

  // ── Combined → Q3.6 (only when at least one sub-result is SERIOUS) ────────
  // (edges from a_ser/b_ser → n36 are shown logically; visual position
  //  is handled by placing n36 between the two sections)
  mkEdge('easer_36', 'a_ser', 'n36', '→ combine'),
  mkEdge('ebser_36', 'b_ser', 'n36', '→ combine'),

  // ── Combined → final outcomes (when no SERIOUS) ──────────────────────────
  // These edges are activated conditionally in the algorithm function below
  mkEdge('ecomb_low',  'a_low', 'o_low', 'both LOW'),
  mkEdge('ecomb_mod',  'a_mod', 'o_mod', 'worst MOD'),

  // ── Q3.6 → ───────────────────────────────────────────────────────────────
  mkEdge('e36_mod',  'n36', 'o_mod', 'Y/PY'),    // corrected for bias → downgrade to MODERATE
  mkEdge('e36_37',   'n36', 'n37',   'N/PN/NI'), // not corrected → check sensitivity analyses

  // ── Q3.7 → ───────────────────────────────────────────────────────────────
  mkEdge('e37_mod',  'n37', 'o_mod', 'Y/PY'),    // sensitivity analyses OK → MODERATE
  mkEdge('e37_38',   'n37', 'n38',   'N/PN/NI'), // not OK → check severity

  // ── Q3.8 → ───────────────────────────────────────────────────────────────
  mkEdge('e38_crit', 'n38', 'o_crit', 'Y/PY'),    // severe bias → CRITICAL
  mkEdge('e38_ser',  'n38', 'o_ser',  'N/PN/NI'), // not severe → SERIOUS
];

// ─── Intermediate result helpers ──────────────────────────────────────────────
// Compute sub-graph A intermediate level from answers
function getSubA(a: Answers): IntermLevel | null {
  if (!a.q31) return null;
  if (a.q31 === 'SY') return 'serious';
  if (a.q31 === 'WN/NI') return 'moderate';
  if (a.q31 === 'Y/PY') {
    if (!a.q32) return null; // still waiting
...
// Compute sub-graph B intermediate level from answers
...
function getActivePath(a: Answers): { nodes: Set<string>; edges: Set<string>; outcome: RiskLevel | null } {
  const nodes = new Set<string>(['n31', 'n33']); // both sub-graphs always shown
  ...
  // ── Combined assessment ────────────────────────────────────────────────
  // Only proceed once BOTH sub-graph results are known
  ...
  if (worst === 'low') {
    ...
    // Worst is MODERATE → final MODERATE
    nodes.add('o_mod'); edges.add('ecomb_mod'); outcome = 'moderate';
  } else {
    // At least one SERIOUS → ask Q3.6
    // Activate the bridge edge from whichever sub-graph is serious
    ...
    if (a.q36 === 'Y/PY') {
      // Corrected for selection bias → downgrade to MODERATE
      ...
      if (a.q37 === 'Y/PY') {
        // Sensitivity analyses show minimal impact → MODERATE
...
function getNextStep(a: Answers): Step {
  ...
  const subA = getSubA(a);
  const subB = getSubB(a);
  if (subA === null || subB === null) return null; // still waiting for sub-results

  const worst = SEVERITY[subA] >= SEVERITY[subB] ? subA : subB;
  if (worst !== 'serious') return null; // no further questions needed
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function Domain3Graph() {
  const [answers, setAnswers] = useState<Answers>({
    q31: null, q32: null, q33: null, q34: null, q35: null,
    q36: null, q37: null, q38: null,
  });
  ...
  // Show sub-graph intermediate results in the panel
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui,sans-serif', background: '#fff7ed' }}>
      <div style={{ background: '#7c2d12', color: '#fff', padding: '12px 20px', flexShrink: 0 }}>
        ...
        <div style={{ fontSize: 12, opacity: 0.5, marginTop: 1 }}>Two sub-graphs (A: follow-up timing, B: selection characteristics) combined</div>
      </div>
      ...
      <div style={{ borderTop: '1px solid #e2e8f0', background: '#fff', padding: '14px 20px', flexShrink: 0, minHeight: 80 }}>
        {/* Show sub-graph progress indicators */}
        {(subA || subB) && (
